    ...
    <>
      ...
      <ul>
        {Object.entries(cityArr).map(([city, value], index) => {
          return [
            <li key={index}> {city} : {value} </li>,
            <button key={index+1} onClick={() => handleDelete(city)} >delete</button>,
            <button key={index+2} onClick={() => handleUpdate(city)} >update</button>
          ];
        })}
      </ul>
    </>
